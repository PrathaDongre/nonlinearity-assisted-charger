...
cx_mat L(double w0, double g1, double g2, double F, int N, cx_mat rho0){
...
cx_mat Rk4(double w0,double g1, double g2, double F, int N, double dt, cx_mat rho)
{
  cx_mat L(double,double,double,double, int, cx_mat);
  cx_mat K1=dt*L(w0,g1,g2,F,N,rho);
  cx_mat K2=dt*L(w0,g1,g2,F,N,rho+0.5*K1);
  cx_mat K3=dt*L(w0,g1,g2,F,N,rho+0.5*K2);
  cx_mat K4=dt*L(w0,g1,g2,F,N,rho+K3);
...
int main()
{
ofstream max_energy_out("max_energy_time_g1_diff_anHO_F_diff.dat");
ofstream max_ergo_out("max_ergotropy_time_g1_diff_anHO_F_diff.dat");

ofstream max_ergotropy_out("AnHO_max_ergotropy_matrix.dat");
// ofstream steady_ergotropy_out("steady_ergotropy_matrix.dat");


int N=10; double g2=0.0,w0=1.06, t=150;
// Array of F values
  vector<double> F_values = {0.07, 0.09 ,0.1, 0.3, 0.5, 0.7, 0.9, 1.1};
  vector<double> g1_values = {0.09, 0.21, 0.33, 0.45, 0.57, 0.69, 0.81, 0.93};

  // Matrix to store maximum energies
  mat max_ergotropy_matrix(F_values.size(), g1_values.size());
  // mat steady_ergotropy_matrix(F_values.size(), g1_values.size());


    // Iterate over F and g2 values for max ergotropy and time--------------------------
  for (size_t i = 0; i < F_values.size(); ++i) {
      double F = F_values[i];
      for (size_t j = 0; j < g1_values.size(); ++j) {
        double g1 = g1_values[j];
        mat I2 = eye(2,2);
        ...
        complex<double> Ergotropy=energy-E_p;
          ...
          if (real(Ergotropy) < max_ergotropy && real(Ergotropy) > 0.1) {
              break;
          }
        }//end of RK4 loop
        
        cout << "For F = " << F << ":" << "and for g1 = " << g1 << ":" <<  endl;
...
complex<double> Ergotropy=energy-E_p;
  ...
  if (real(energy) > max_energy) {
    ...
    // Break the loop if energy and ergotropy drop after reaching their maximum values
  if (real(energy) < max_energy && real(energy) > 0.1*w0) {
      break;
  }
} //--------end of RK4 loop
cout << "For F = " << F << ":" <<"For fixed g1 = "<< g1<< endl;
